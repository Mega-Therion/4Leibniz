...
class QueueManager:
    def __init__(self, database: str | Path):
        ...
        with self._connect() as db:
            db.executescript("""CREATE TABLE IF NOT EXISTS jobs (id TEXT PRIMARY KEY, kind TEXT NOT NULL, payload TEXT NOT NULL, status TEXT NOT NULL DEFAULT 'pending', lease_owner TEXT, result TEXT, created_at REAL NOT NULL); CREATE TABLE IF NOT EXISTS responses (job_id TEXT NOT NULL, worker_id TEXT NOT NULL, response TEXT NOT NULL, created_at REAL NOT NULL, PRIMARY KEY (job_id, worker_id));""")
    def _connect(self):
    ...
    def record_response(self, job_id: str, worker_id: str, response: dict[str, Any]) -> None:
        with self._connect() as db: db.execute("INSERT OR REPLACE INTO responses VALUES (?, ?, ?, ?)", (job_id, worker_id, json.dumps(response, sort_keys=True), time.time()))
    ...
    def consensus(self, job_id: str, minimum_workers: int = 3, threshold: float = 0.95) -> dict[str, Any]:
        with self._connect() as db: rows = db.execute("SELECT response FROM responses WHERE job_id=? ORDER BY worker_id", (job_id,)).fetchall()
        responses = [json.loads(row['response']) for row in rows]
        if len(responses) < minimum_workers: return {"status": "pending-review", "responses": len(responses), "reason": "minimum independent workers not reached"}
        ...
        texts = [str(response.get('text', '')) for response in responses]; best = max(set(texts), key=texts.count); agreement = sum(1 for text in texts if text == best) / len(texts)
